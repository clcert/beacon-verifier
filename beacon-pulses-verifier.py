...
pulses_group.add_argument("--tail", action="store", dest="tail_pulses", type=int, default=0,
                          help="Checks only the last TAIL_PULSES pulses generated by the beacon.")
...
elif options.tail_pulses != 0:
    pulses_to_verify = range(li - options.tail_pulses + 1, li + 1)
else:
...
elif options.tail_pulses:
    vprint("Pulses from #" + str(li - options.tail_pulses + 1) + " to #" + str(li))
else:
    vprint("Pulses from #" + str(options.init_id) + " to #" + str(options.final_id))

prime_p = None
prev_comm = prev_value = None
for i in tqdm(pulses_to_verify, unit='pulses', desc='Progress', disable=not options.verbose):
